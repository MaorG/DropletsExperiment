...
%                     end
        ...
        function writeNewLoadFile(obj, fieldNames, allFileNames, loadFileName, loadStructName)
                ...
                for ni = 1:numel(allFieldNames)
                    ...
                    if ~isnumeric(entry) || isempty(entry)
                        if (isstruct(entry) || iscell(entry) || isempty(entry))
                            loadStruct(di).(allFieldNames{ni}) = entry;
                            loadConfig(di).(allFieldNames{ni}) = ['loadStruct(', num2str(di), ').', allFieldNames{ni}];
                        else
        ...
        function props = doExportImagesParameterExtraction(obj, v)
            ...
            for i = 1:numel(v)
                ...
                elseif (strcmp(v{i}, 'structLoad'))
                    props.structLoad = v{i+1};                    
                end
            end
