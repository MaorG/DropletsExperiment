    ...
    methods
        ...
        function doOutputRow(obj, outputConfigRow)
            ...
            elseif strcmpi('defaultParams', outputConfigRow.stage)
                if (isfield(outputConfigRow, 'parameters') && ~isempty(outputConfigRow.parameters))
                    addDefault(obj, 'defaultOutputParams', {{{outputConfigRow.funcName} {outputConfigRow.parameters}}});
                end    
            elseif strcmpi('defaultFigPostOps', outputConfigRow.stage)
                if (isfield(outputConfigRow, 'parameters') && ~isempty(outputConfigRow.parameters))
                    addDefault(obj, 'defaultOutputFigPostOps', {{{outputConfigRow.funcName} {outputConfigRow.parameters}}});
                end
            end
        ...
        function doOutputRowData(obj, outputConfigRow)
            ...
            if ~(isfield(outputConfigRow, 'hold') && ~isempty(outputConfigRow.hold))
                ...
                holdArray = {};
                for i = 1:numel(fData)
                    if sum(strcmp(fData(i).parameters.(outputConfigRow.hold{1}),holdArray))==0
                        holdArray{end+1} = fData(i).parameters.(outputConfigRow.hold{1});
                        
                        entry = fData(i);
                        str = obj.getTitle2(entry, filter(1:2:end), outputConfigRow);
                        figureArray(end+1) = figure('Name', str);
                        ...
                        idx = find(strcmp(fData(i).parameters.(outputConfigRow.hold{1}),holdArray))
                        entry = fData(i);
                        figure(figureArray(idx))
                        hold on;
                        if (~isempty(outputConfigRow.srcName))
                            eval([funcName '(entry.(outputConfigRow.srcName), parameters)']);
                        else
        ...
        function addDefault(obj, field, params)
            obj.(field) = [obj.(field), params];
                ...
                for gi = 1:numel(entryGroups)
                    ...
                    if ~isempty(entryGroups{gi})
                        ...
                        if (isfield(outputConfigRow, 'figPostOps'))
                            % used when there is an additional configuration field of
                            % 'figPostOps' to be used not as default but as specific properties
                            % to be added to an output line (default are appended)
                            origOps = outputConfigRow.figPostOps;
                        else
                            origOps = cell(0);
                        end
                        ops = origOps;
                        % use 'empty' in figPostOps when setting
                        % default parameters to remove all previous
                        % added parameters for the specified functions
                        % and start anew
                        
                        for i = 1 : numel(obj.defaultOutputFigPostOps)
                            defOps = obj.defaultOutputFigPostOps{i};
                            funcs = defOps{1}{1};
                            params = defOps{2}{1};
                            if (isempty(funcs) || any(strcmp(funcs, outputConfigRow.funcName)))
                                for p = 1 : numel(params)
                                    curParam = params{p};
                                    if (strcmp(curParam, 'empty'))
                                        ops = origOps;
                                    else
                                        ops = [ops, curParam];
                                    end
