...
%             obj.(entName) = st;
            ...
            for i = 1:numel(obj.dm.allData)
                ...
                st.dataHandle = obj.dm.allData(i);
            ...
            for i = 1 : numel(ents)
                if (isfield(ents(i).dataHandle.properties, 'prevUID'))
                    prevDataIdx = find(allDataIDs == ents(i).dataHandle.properties.prevUID);
                    ents(i).dataProperties.prevEntity = ents(prevDataIdx);
                end
                if (isfield(ents(i).dataHandle.properties, 'nextUID'))
                    nextDataIdx = find(allDataIDs == ents(i).dataHandle.properties.nextUID);
