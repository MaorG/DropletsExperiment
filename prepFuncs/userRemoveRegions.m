...
while userinput ~= 13
    ...
    if (isfield(props, 'display') && ~isempty(props.display))
        eval([props.display{1} '(data, props.display{2}, AL)']);
    else
        imshow(imfuse( ...
            cat(3,...
            mat2gray(BFUI,clim), ...
    ...
    % ability to zoom in/out until space bar is pressed
    set(gcf, 'CurrentCharacter', 'a');
    set(h,'Name','Zoom in/out and then press spacebar','NumberTitle','off');
